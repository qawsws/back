...
<div id="wrap">
	...
	<div id="container">
		...
		<div class="bodytext_area box_inner">
			...
			<c:set var="totalPages" value="${(totalCount / perPage) + (totalCount % perPage > 0 ? 1 : 0)}" />
			
			<c:if test="${currentPage > 1}">
				<a href="boardlist.do?page=1&searchWord=${fn:escapeXml(searchWord)}">«</a>
			    <a href="boardlist.do?page=${currentPage - 1}&searchWord=${fn:escapeXml(searchWord)}">‹</a>
			</c:if>
			...
			<c:forEach begin="1" end="${totalPages}" var="i">
			  ...
			  <c:otherwise>
			      <a href="boardlist.do?page=${i}&searchWord=${fn:escapeXml(searchWord)}">${i}</a>
			  </c:otherwise>
			  </c:choose>
			</c:forEach>
			...
			<c:if test="${currentPage < totalPages}">
			  <a href="boardlist.do?page=${currentPage + 1}&searchWord=${fn:escapeXml(searchWord)}">›</a>
			  <a href="boardlist.do?page=${totalPages}&searchWord=${fn:escapeXml(searchWord)}">»</a>
			</c:if>
			</div>			
<!-- //pagination -->
