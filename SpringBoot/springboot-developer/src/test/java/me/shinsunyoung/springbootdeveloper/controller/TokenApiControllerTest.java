...
public class TokenApiControllerTest {
    ...
    public void createNewAccessToken() throws Exception {
        ...
        User testUser = userRepository.save(User.builder()
                ...
                .password("test")
                .build());
        // 정상적인 RefreshToken 생성
        String refreshToken = JwtFactory.builder()
                .claims(Map.of("id",testUser.getId()))
                .build()
                .createToken(jwtProperties);
        // 데이터베이스에 RefreshToken을 저장
        refreshTokenRepository.save(new RefreshToken(testUser.getId(), refreshToken));
        // createNewAccessToken 메서드를 실행하기 위한 데이터를 생성
        CreateAccessTokenRequest request = new CreateAccessTokenRequest();
        // 위에서 만든 정상적인 RefreshToken을 request에 저장
        request.setRefreshToken(refreshToken);
        final String requestBody = objectMapper.writeValueAsString(request);
        // TokenApiController의 createNewAccessToken메서드를 실행
        ResultActions resultActions = mockMvc.perform(post(url)
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody));
        // 실행 결과를 확인, 응답코드가 create면, accessToken이 있으면 통과
